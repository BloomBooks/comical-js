...
export class Comical {
    ...
    static movePointOutsideBubbleContent(element: HTMLElement, position: Point): Point {
        ...
        while (bubble) {
            ...
            delta = Comical.betterDelta(parentContainer, result, delta, new Point(result.x!, bounds.top - 1)); // up
            delta = Comical.betterDelta(parentContainer, result, delta, new Point(bounds.left - 1, result.y!)); // left
            delta = Comical.betterDelta(parentContainer, result, delta, new Point(bounds.right + 1, result.y!)); // right
    ...
    // If neither option is outside a bubble, return delta1.
    private static betterDelta(element: HTMLElement, start: Point, delta1: Point, candidate: Point): Point {
        ...
        // (1) it is closer, or
        // (2) start is NOT OK.
        if (okToMoveTo2 && (delta2IsCloser || !this.okToMoveTo(element, start.add(delta1)))) {
            return delta2;
        }
        // this covers cases where delta1 is closer, where it's OK but delta2 is not,
        // and where neither is OK.
        return delta1;
    }

    private static okToMoveTo(element: HTMLElement, dest: Point): boolean {
        return !this.bubbleWithContentAtPoint(element, dest.x!, dest.y!);
    }
