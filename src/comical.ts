...
export class Comical {
    ...
    // Removes the bubble associated with 'activeElement' from its 'family' of bubbles.
    // This can either be the parent(or patriarch) of this family or one of the child bubbles.
    public static deleteBubbleFromFamily(activeElement: HTMLElement, container: HTMLElement) {
        const specToDelete = Bubble.getBubbleSpec(activeElement);
        ...
        if (orderOfGoner && orderOfGoner > 0) {
            ...
            const tempBubble = new Bubble(activeElement);
        ...
        container.removeChild(activeElement);
