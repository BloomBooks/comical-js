...
export class Comical {
    ...
    public static convertCanvasToSvgImg(parent: HTMLElement) {
        ...
        if (bubbles.length !== 0 && Comical.atLeastOneBubbleIsVisible(bubbles)) {
    ...
    private static atLeastOneBubbleIsVisible(bubbles: Bubble[]): boolean {
        for (let i = 0; i < bubbles.length; i++) {
            if (!Bubble.bubbleIsTransparent(bubbles[i])) {
                return true;
            }
        }
        return false;
