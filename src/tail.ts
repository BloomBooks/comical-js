...
export class Tail {
  ...
  public makeShapes() {
    ...
    deltaMidLine.length = 3000;
    const end1 = centerRootTip.add(deltaMidLine);
    const end2 = centerRootTip.subtract(deltaMidLine);
    const tempLine = new Path.Line(end1, end2);
    ...
    const intersect = tempArc.getIntersections(tempLine);
    ...
    // one...maybe it's more than 3000px out? Anyway using an unmodified
  ...
  public showHandles() {
    const curveHandle = this.makeHandle(this.mid, true);
    ...
    if (!this.spec.joiner) {
      ...
      tipHandle = this.makeHandle(this.tip, false);
