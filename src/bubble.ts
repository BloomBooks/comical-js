...
export class Bubble {
    ...
    private oulineStrokeColor: Color | null = null;
    ...
    // bubble family.
    public combinedShapes: Item | undefined = undefined;
    ...
    public fillArea: Item | undefined;
    ...
    private makeShapes(shape: Item) {
        ...
        if (outlineShape) {
            shape.remove();
            this.setOutline((outlineShape as Shape).toPath());
    ...
    private setOutline(outline: Item): void {
        this.outline = outline;
        this.oulineStrokeColor = outline.strokeColor;
    }

    public uniteShapes() {
        const relatives = Comical.findRelatives(this, true);
        if (relatives.length === 0) {
            // just possible if this has no order
            relatives.push(this);
        }
        ...
        if (relatives.some(r => !r.outline || !(r.outline as PathItem).unite)) {
        ...
        let combinedPath: PathItem = parent.outline.clone({ insert: false }) as PathItem;
        ...
        for (var i = 1; i < relatives.length; i++) {
            if (!relatives[i].isOverlapping(relatives[i - 1])) {
                // child tails should overlap the shape we already have.
                // Joining the tail first may help to keep the shape simple,
                // or at least contiguous. Not sure this is necessary, unite()
                // seems to handle discontiguous shapes.
                combinedPath = relatives[i].uniteTails(combinedPath, tailsToClip);
            } else {
                // if the bubbles overlap we don't need the joining tail; just hide it.
                relatives[i].tails.forEach(t => this.hideShape(t.pathstroke));
            }
            // We can be confident the outline is a PathItem because we checked
            // above that it has the unite() function.
            combinedPath = combinedPath.unite(relatives[i].outline as PathItem);
            this.hideShape(relatives[i].outline);
        }
        const style = this.getStyle();
        ...
        if (tailsToClip.length) {
            const groupItems: PathItem[] = [combinedPath];
            tailsToClip.forEach(t => {
                let clippedTail = t.pathstroke.clone({ insert: false }) as PathItem;
                ...
                relatives.forEach(r => {
                    ...
                    if (r.tails.includes(t)) {
                        const outlinePath = r.outline as PathItem;
            ...
            parent.combinedShapes = new Group(groupItems);
        }
    ...
    uniteTails(combinedPath: PathItem, tailsToClip: Tail[]): PathItem {
    ...
    public static almostInvisibleColor = new Color(1, 1, 1, 0.001);
    ...
    public isHitByPoint(point: Point): boolean {
        if (!this.fillArea) {
            if (this.outline.fillColor && this.outline.fillColor.alpha) {
